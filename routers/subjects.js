...
app.route('/create').post(async (req, res) => {
    try {
        const body = req.body;
        const createQuery = await pool.query(`INSERT INTO subjects (course, subject) VALUES ($1, $2) RETURNING *`, [body.course, body.subject]);
...
app.route('/update/:course').put(async (req, res) => {
    ...
    try {
        let body = req.body;
        const updateQuery = await pool.query(`UPDATE subjects SET subject = $1 WHERE course = $2`, [body.subject, course]);
